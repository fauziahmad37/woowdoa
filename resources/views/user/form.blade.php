				...
				<ol class="list-reset flex flex-wrap items-center">
						...
						<li>
								<span class="text-gray-500">
										{{ isset($user) ? 'Edit Data User' : 'Tambah User' }}
								</span>
						</li>
				</ol>
		</nav>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
	...
	<div>
			...
			<input type="text" name="complete_name"
						 value="{{ old('complete_name', $user->complete_name ?? '') }}"
	...
	<div>
			<label class="block text-gray-700 font-medium mb-1">
					Password (Kosongkan jika tidak diganti)<span class="text-red-500">*</span>
			</label>
	...
	<label class="block text-gray-700 font-medium mb-1">Profil Foto</label>
		@if(isset($user) && optional($user)->profile_photo)
		<img src="{{ $user && $user->profile_photo
		? asset('storage/' . $user->profile_photo)
		: asset('images/default-avatar.png') }}"
		class="w-12 h-12 object-cover rounded-full border">
		@endif

		<input type="file" name="profile_photo" 	class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-green-500">
	</div>
