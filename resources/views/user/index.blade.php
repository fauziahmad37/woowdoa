...
<div class="py-6">
    <div class="container mx-auto w-full">
        ...
        <!-- Header -->
        <h2 class="text-xl font-semibold text-gray-600">Kelola User</h2>
        <p class="text-gray-500 mb-10">Pendataan User</p>
            ...
            @if($user->count() > 0)
                ...
                <div class="w-full overflow-x-auto p-4">
                    <table class="min-w-full bg-white border border-gray-200 divide-y divide-gray-200 table-auto">
                        ...
                        <tbody>
                            @foreach($user as $i => $rec)
                                <tr class="border-b hover:bg-gray-50">
                                    <td class="px-4 py-3">
                                        {{ ($user->currentPage() - 1) * $user->perPage() + $i + 1 }}
                                    </td>
                                    <td class="px-4 py-3">{{ $rec->complete_name }}</td>
                                    <td class="px-4 py-3">{{ $rec->username }}</td>
                                    <td class="px-4 py-3">{{ $rec->level->user_level_name }}</td>
                                    <td class="px-4 py-3">{{ $rec->email }}</td>
                                    <td class="px-4 py-3">{{ $rec->phone }}</td>
                                    <td class="px-4 py-3">
																				<img src="{{ $rec->profile_photo }}" width="50" />
																				</td>
                                    <td class="px-4 py-3">
                                        @if($rec->active)
